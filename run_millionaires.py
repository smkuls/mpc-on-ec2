...
def run_viff_millionaires(instance_ids, instance_ips):
    ...
    viff_export_command =  "echo 'export PYTHONPATH=$PYTHONPATH:$HOME/opt/lib/python' >> ~/.bashrc"
    source_bashrc_command = "source ~/.bashrc"
    for player_number, instance_id in zip(range(1, ec2Manager.config.VM_COUNT + 1), instance_ids):
        viff_config_command = "python ~/viff-f1d477e94d0b/apps/generate-config-files.py -n "+str(ec2Manager.config.VM_COUNT)+" -t 1"
        for port_number, instance_ip in zip(port_numbers, instance_ips):
            viff_config_command += " " + instance_ip+":"+str(port_number)

        viff_app_command = "python ~/viff-f1d477e94d0b/apps/millionaires.py --no-ssl player-"+str(player_number)+".ini > output 2>&1 &"

        # print (viff_config_command)
        # print (viff_app_command)
        ec2Manager.execute_command_on_instance(instance_id, [viff_export_command, source_bashrc_command,
